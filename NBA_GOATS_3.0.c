 ...
 //Inicio programa es un printf que muestra en la pantalla las distintas opciones para continuar	
	...
	switch (opcion){
		case 1: 
			...
			//Arimos un fichero para guardar los usuarios y sus respectivas conntraseñas si no existe el fichero se crea
...
void infojugadoreseste(){
	...
	// Lee el fichero
while(fscanf(pj,"%[^;];%[^;];%f;%f;%f;%[^;];%i\n",cr[i].nombre,cr[i].posicion,&cr[i].puntos,&cr[i].asistencias,&cr[i].rebotes,cr[i].equipo,&cr[i].lista) != EOF)
    i++;
    ...
    do{
	printf("\n16.Volver al menu\n");
    printf("\nEscribe el equipo que quieres que se vea o regresa al menu: ");
    
    scanf("%d", &equipo);
	} while (equipo<1 || equipo>16);
	if (equipo==16){
		system ("cls");
		fflush(stdin);
		menu();
	...
	do{
	fflush(stdin);
	printf("\nEscribe el nombre de tu jugador preferido:");
	printf("\n( respeta las mayusculas :D )");
	...
	vd= strcmp(xl,cr[j].nombre );
   
    if (vd==0)
		
		{

		printf("\nNombre: %s\nPosicion: %s\nPuntos: %.2f\nAsistencias: %.2f\nRebotes: %.2f\n",cr[j].nombre, cr[j].posicion,cr[j].puntos,cr[j].asistencias,cr[j].rebotes);
		break;
		}
